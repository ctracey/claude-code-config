...
Reference docs use the existing tracking file convention — no separate files needed. All scoped to the PR number.

| File | Contents |
|---|---|
| `.claude/todo-pr-N.md` | Numbered task breakdown with descriptions and key context |
| `.claude/todo-pr-N_plan.md` | Feature list, goals, product intent, out of scope |
| `.claude/todo-pr-N_architecture.md` | Tech stack decisions, folder structure, architectural constraints |

### `todo-pr-N.md` task format

```markdown
## Task N: [name]
[What needs doing]
**Context:** [constraints, gotchas, decisions already made]
**Done when:** [specific acceptance criteria]
```

### `todo-pr-N_plan.md` format
...
### `todo-pr-N_architecture.md` format
