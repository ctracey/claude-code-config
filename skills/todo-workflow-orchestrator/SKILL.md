---
name: todo-workflow-orchestrator
description: Generic workflow orchestrator — drives a user through a sequence of stages defined by the calling workflow skill. Manages progress banner, stage gates, and skill invocation. Use when a workflow skill hands off a workflow definition to run, or when invoked via /todo-workflow-orchestrator.
...
1. **Emit progress banner** — invoke `todo-workflow-progress` with:
